...
function defaultProfile(slug: string, name: string, zhName: string, provinceSlug: string, provinceName: string, zhProvinceName: string, provinceTags: string[]): Required<CityProfile> {
  ...
  return {
    ...
    summary: `Explore ${name}: tap to see its universities, cost of living, student-life signals, weekend travel context, and application planning notes.`,
    zhSummary: `探索${zhName}：点击查看当地大学、生活成本、学生生活信号、周末旅行背景和申请规划提示。`,
