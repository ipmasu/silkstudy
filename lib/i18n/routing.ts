export const supportedLocales = ["en", "zh", "vi", "fr", "ko", "th", "id", "ms", "my", "km", "lo", "tl", "ru", "tr"] as const;
...
export function localizePath(path: string, locale: string) {
  ...
  const pathWithoutLocale = normalizedPath.replace(/^\/(en|zh|vi|fr|ko|th|id|ms|my|km|lo|tl|ru|tr)(?=\/|$)/, "") || "/";
